...
class Empty extends TweetSet {
  ...
  def mostRetweeted: Tweet =
    throw new java.util.NoSuchElementException(
      "mostRetweeted of Empty TweetSet"
    )
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def union(that: TweetSet): TweetSet = {
    ...
    else filterAcc(t => true, that)
  }

  def mostRetweeted: Tweet = {
    def max(t1: Tweet, t2: Tweet) = if (t1.retweets > t2.retweets) t1 else t2
    def maxByRetweet(t: Tweet, ts: TweetSet) =
      if (ts.isEmpty) t else max(t, ts.mostRetweeted)

    maxByRetweet(maxByRetweet(elem, left), right)
  }
